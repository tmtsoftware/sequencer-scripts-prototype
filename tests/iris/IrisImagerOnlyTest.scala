...
import ocs.testkit.mocks.SequencerFactory
...
class IrisImagerOnlyTest extends FunSuite {
  test("test handler for command setupObservation") {
    ...
    val eventualResponse = irisImagerOnly.execute(
      Setup(Prefix("sequencer"),
            CommandName("setupObservation"),
            None,
            Set(filterKeyParamset, itimeKeyParamset, rampsKeyParamset))
    )
